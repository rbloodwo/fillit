...
void 	assign_board_mem(char **board, boardinfo *binfo) // ! fix this
{
	int i;
	int j;
	...
	// add a function to get each piece then assign it to each first index
	board[0] = "!?";
	board[1] = "##\n #\n #"; // this actually works. build around it
	//board[0][0] = '?';
	ft_putstr(board[1]);
	ft_putchar(board[0][1]);
	//ft_putchar(board[0][0]);
}
